...
export default function Home() {
  ...
  return (
    <main
      className={` flex overflow-fit items-center justify-center h-screen `}
    >
      <div
        className={`fixed top-0 right-0 my-auto px-5 w-screen h-screen border-2 flex flex-col z-10 text-center justify-center text-4xl font-Just_Me_Again_Down_Here rounded-xl shadow-xl bg-pkemon-yellow ${
          orientation === "portrait" ? "" : "hidden"
        }`}
      >
        <p>
          This websites works best in landscape mode.{" "}
          <span className="text-5xl"> please rotate your device!!</span>{" "}
        </p>
      </div>
      <div
        className={`  flex w-full h-full lg:flex-col items-center ${
          orientation === "portrait" ? "hidden" : "flex-row "
        }`}
      >
        <div
          className={`ml-10 -mr-6 basis-1/4 h-full w-full flex flex-col items-center justify-center lg:my-10 lg:ml-0 lg:-mr-0 `}
        >
          <Image
            className="w-44 xl:w-56 2xl:w-72"
            src="/logo.png"
            width={300}
            height={300}
          />
          <p className="text-2xl lg:text-3xl 2xl:text-4xl font-Just_Me_Again_Down_Here ">
            Advanced Edition
          </p>
        ...
        <div
          className={`basis-3/4 flex flex-col justify-center lg:w-3/4 xl:w-3/5 2xl:w-1/2 `}
        >
          <div className="bg-pokedex bg-no-repeat bg-center bg-contain aspect-video flex flex-row items-center justify-ar">
            <div className="dpad-container grid grid-cols-2 rotate-45 border-0 ">
              <div className="center-circle absolute bg-primary-blue"></div>
              <button
                className="dpad-buttons bg-dpad-grey flex items-center justify-center"
                onClick={() => handleButtonPress("up")}
              ></button>
              <button
                className="dpad-buttons bg-dpad-grey"
            ...
            <div className="screen-container border-0 flex">
              <Screen buttonPress={buttonPress[0]} times={buttonPress[1]} />
            </div>
            <div className="button-container border-0 rotate-0">
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="absolute side-buttons-2 "
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="absolute side-buttons-3 "
